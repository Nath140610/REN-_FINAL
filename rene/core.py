...
# ============================================================
# RENÉ L'INTÉRIMAIRE — MOTEUR CENTRAL (ARCHITECTURE MODULAIRE)
# ============================================================
#
...
class ReneCommandTree(app_commands.CommandTree):
    ...
    async def on_error(
        ...
        arguments: dict[str, Any] = {"embed": embed, "ephemeral": True}
...
class ReneBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        # Charge automatiquement tous les fichiers Python de rene/commands et rene/events.
        # Pour ajouter une nouvelle commande, il suffit donc d'ajouter son fichier :
        # le script principal n'a jamais besoin d'être modifié.
        from rene.loader import load_all_extensions

        loaded = await load_all_extensions(self)
        logger.info("%s module(s) René chargé(s) automatiquement.", len(loaded))
    ...
    async def reload_rene_modules(self) -> tuple[list[str], list[str], list[str]]:
        """Recharge les modules existants et détecte les nouveaux fichiers."""
        from rene.loader import reload_all_extensions
        return await reload_all_extensions(self)
